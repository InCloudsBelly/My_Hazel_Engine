...
namespace MyHazel
{
	EditorLayer::EditorLayer()
		:Layer("EditorLayer"), m_CameraController(1280.0f / 720.0f), m_ImGuiColor({0.2f,0.3f,0.8f,1.0f})
	{
	}
	void EditorLayer::OnAttach()
	{
		...
		MyHazel::FramebufferSpecification fbSpec;
		fbSpec.Width = 1280;
		fbSpec.Height = 720;
		m_FrameBuffer = MyHazel::Framebuffer::Create(fbSpec);
	}
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		//Resize
		if (MyHazel::FramebufferSpecification spec = m_FrameBuffer->GetSpecification();
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Exit")) MyHazel::Application::Get().Close();
				ImGui::EndMenu();
			}
