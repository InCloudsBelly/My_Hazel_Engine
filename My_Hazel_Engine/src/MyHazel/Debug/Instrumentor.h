...
#define HZ_PROFILE 0
	...
	#define HZ_PROFILE_BEGIN_SESSION(name, filepath) ::MyHazel::Instrumentor::Get().BeginSession(name,filepath)
	#define HZ_PROFILE_END_SESSION() ::MyHazel::Instrumentor::Get().EndSession()
	#define HZ_PROFILE_SCOPE(name) ::MyHazel::InstrumentationTimer timer##__LINE__(name)
